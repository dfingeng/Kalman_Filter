import cv2
import numpy as np
from pykalman import KalmanFilter #导入库pykalman
...
kf = KalmanFilter(transition_matrices=np.array([[1, 0, 1, 0], [0, 1, 0, 1], [0, 0, 1, 0], [0, 0, 0, 1]]),
                  ...
                  transition_covariance=0.03 * np.eye(4)) #np.eye(4) 单位矩阵
...
def mousemove(event, x, y, s, p):
    ...
    filtered_state_means, filtered_state_covariances = (kf.filter_update(filtered_state_means0
                                                        ,filtered_state_covariances0, current_measurement))
...
while True:
    ...
    if (cv2.waitKey(30) & 0xff) == 27:  #按ESC退出窗口
        break
